...
{
    if (the_dumpfile) {
        cerr << "SystemC warning: dumpfile has already been specified to `"
             << dumpfile_name << "'" << endl;
        cerr << "SystemC warning: request ignored" << endl;
    }
    else {
        if (fname && fname[0]) {
            ...
            cerr << "SystemC info: dumpfile name not specified, using `"
                 << dumpfile_name << "'" << endl;
        }
...
sc_dumpvars(const char* objs_string)
{
    ...
    while (*q) {
        ...
        if (obj)
            ...
            cerr << "SystemC warning: object `" << r << "' not found." << endl;
