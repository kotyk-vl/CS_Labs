...
#else
  print_pln_object;  // print legal notice
#endif
*/
...
sc_simcontext::initialize()
{
	...
	trace_cycle( /* is not delta cycle */ false );
        ...
        if (NULL == ConvertThreadToFiber(NULL)) {
            cout << "SystemC Error: Unable to convert thread to fiber (Windows NT)." << endl;
            error_occurred = true;
        }
#endif
    }
    ...
    if (error_occurred) {
        cout << "SystemC Info: Simulation cannot begin due to previous errors." << endl;
        exit(1);        
    }
}
