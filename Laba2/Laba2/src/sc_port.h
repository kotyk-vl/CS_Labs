...
public:
    const T& read() const     { return const_signal()->read();  }
    operator const T&() const { return const_signal()->read();  }
    const bool event() const  { return const_signal()->event(); }
...
class sc_out : public sc_port<T> {
    ...
    {
        nonconst_signal()->write(sig.read());
        return *this;
    }
...
public:
    const data_type& read() const     { return const_signal()->read();  }
    operator const data_type&() const { return const_signal()->read();  }
    const bool event() const  { return const_signal()->event(); }
