...
#ifdef SC_UINT32_H
#ifndef SC_UINT32_DEFS_H
#define SC_UINT32_DEFS_H
...
{
  width = v.left - v.right + 1; num = v.operator unsigned int();
}
...
inline unsigned int sc_uint_base::range( int left, int right ) const
{
  if( left < width - 1 )
    return ( (num & ((1U << (left + 1)) - 1)) >> right ); 
  else
    return num >> right; 
}
