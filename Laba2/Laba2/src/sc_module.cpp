      ...
      sensitive_neg(this)
{
    ...
    if (0 == mod_name || 
        0 != mod_name->module) {
        cerr << "An sc_module_name parameter for your constructor is required" << endl;
        exit(1);
    }
...
sc_module::validate_all()
{
    ...
    for ( ; ! it.empty(); it++) {
        sc_module* p = *it;
        if (! p->validate()) {
            cerr << "SystemC error: did you forget to call end_module() for `"
                 << p->name() << "'?\n";
            if (error) {
                cerr << "  (Note: Hierarchical name as shown may be incorrect "
                     << "due to previous errors)" << endl;
            }
            error = true;
        }
    }
...
sc_module::set_stack_size(size_t size)
{
  ...
  if (SC_CURR_PROC_SYNC == cpi->kind) {
    ...
    cerr << "SystemC error: You can set the stack size for only synchronous and asynchronous thread processes." << endl; 
}
...
sc_module::operator<<(const sc_signal_base& s)
{
    if (port_vector->size() <= port_index) {
        cerr << "SystemC warning: Cannot bind an extra signal, "
             << s.name() << ", to module " << name() 
             << ". The extra is ignored." << endl;
        return *this;
    }
...
{
    if (port_vector->size() <= port_index) {
        cerr << "SystemC warning: Cannot bind an extra port to module "
             << name() << ". The extra is ignored." << endl;
        return *this;
    }
