                           ...
                           const sc_port_b* to_port )
{
    ...
    if (PORT_STATUS_UNBOUND == ps) {
        ...
        if(curr_module==NULL)
            throw "port specified outside of module";
...
sc_port_manager::resolve_all_ports()
{
    ...
    for ( ; !it.empty(); it++) {
        ...
        case PORT_STATUS_UNBOUND: { 
            cout << "SystemC Error: Port ";
            if (port_info->container) {
                cout << (port_info->ordinal + 1)
                     << " of module `" << port_info->container->name() << "'";
            } else {
                cout << port;
            }
            cout << " is unbound" << endl;
            flag_error();
            break;
        }
        ...
        case PORT_STATUS_BOUND_PORT: {
            ...
            if (PORT_STATUS_BOUND_SIGNAL == to_port_info->port_status) {
                ...
                cout << " is unbound because port ";
                if (to_port_info->container) {
                  cout << (to_port_info->ordinal + 1)
...
sc_port_manager::resolve_unbound_syncs()
{
    ...
    for (curr = unbound_syncs;
        ...
        if ( 0 == clock ) {
            ...
            cout << " must eventually be bound to a clock signal" << endl;
            assert(false);
        }
...
{
    cout << "SystemC Error: Port " << port_index
    ...
    case BIND_STATUS_REBIND:
        cout << " has already been bound, cannot be bound again" << endl;
        break;

    case BIND_STATUS_WRONG_TYPE:
        cout << " is being bound to a signal/port of incompatible type" << endl;
        break;

    case BIND_STATUS_IN_TO_OUT:
        cout << " is an in-port, but being bound to an out-port" << endl;
        break;

    case BIND_STATUS_OUT_TO_IN:
        cout << " is an out-port, but being bound to an in-port" << endl;
        break;

    case BIND_STATUS_INOUT_TO_NON_INOUT:
        cout << " is an inout-port, but being bound to an in-port or out-port" << endl;
        break;

    case BIND_STATUS_NON_IN_TO_CONST_SIG:
        cout << " is an out- or inout-port, but being bound to a const signal" << endl;
        break;

    case BIND_STATUS_UNKNOWN:
        cout << " is causing an unknown error while being bound" << endl;
        break;
    }
}
