...
sc_object::sc_object_init(const char* nm)
{
    ...
    if (put_in_table) {
        if (object_manager->find_object(m_name)) {
            cerr << "SystemC error: Object `" << m_name << "' already exists." << endl;
            cerr << "              (Latter declaration will be ignored.)" << endl;
        }
        else {
...
sc_object::sc_object(const char* nm)
{
    ...
    if (nm && sc_enable_name_checking) {
        ...
        *q = '\0';
        p = namebuf;
        if (has_illegal_char) {
            cerr << "SystemC warning: Illegal characters; `" << nm
                 << "' substituted by `" << namebuf << "'\n";
        }
    }
    sc_object_init(p);
