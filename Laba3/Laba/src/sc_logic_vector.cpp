...
#ifndef _MSC_VER
template <class T>
static T min(T t1, T t2) { return t1>t2?t2:t1;}
#endif
...
{
    return DCAST(sc_logic_vector&, sc_array_base::sub( i, j ));
}
...
sc_logic_vector::operator=( const sc_bool_vector& nv )
{
    ...
    for (int i = length() - 1; i >= 0; --i) {
        assign_help( i, char(*nv.get_data_ptr(i)) );
    }
    return *this;
}
...
#include "sc_logic_vector_and.cpp"
#include "sc_logic_vector_or.cpp"
#include "sc_logic_vector_xor.cpp"
...
sc_logic_vector::to_signed() const
{
    ...
    if ((unsigned)len > sizeof(int)*8) {
		cerr << "sc_logic_vector::to_signed(): " << endl;
        cerr << "    warning: vector is wider than int" << endl;
        cerr << "             value may be truncated" << endl;
        v = 0;
...
sc_logic_vector::to_unsigned() const
{
    ...
    unsigned v = 0;
    if ((unsigned) len > sizeof(int)*8) {
        cerr << "sc_logic_vector::to_unsigned():" << endl;
        cerr << "    warning: vector is wider than unsigned" <<endl;
        cerr << "    value may be truncated" <<endl;
        for (int i = sizeof(int)*8 - 1; i >= 0; --i) {
            v = (v << 1) + int(get_data_ptr(i)->to_bool());
        }
...
{
    return DCAST(sc_signal_logic_vector&, sc_array_base::sub(i,j));
}
...
sc_signal_logic_vector::sc_signal_logic_vector( signal_sulv* v1, signal_sulv* v2 )
    : super( v1, v2 )
{
    /*EMPTY*/
}

sc_signal_logic_vector::sc_signal_logic_vector( signal_sulv* v1, sc_signal<sc_logic>* v2 )
    : super( v1, v2 )
{
    /*EMPTY*/
}

sc_signal_logic_vector::sc_signal_logic_vector( sc_signal<sc_logic>* v1, signal_sulv* v2 )
    : super( v1, v2 )
{
    /*EMPTY*/
}
...
sc_logic_vector&
operator,( sc_logic_vector& x, sc_logic_vector& y )
{
    return *(sc_logic_vector::get_concat_array( &x, &y ));
}

sc_logic_vector&
operator,( sc_logic_vector& x, sc_logic& y )
{
    return *(sc_logic_vector::get_concat_array( &x, &y ));
}
...
const sc_logic_vector&
operator,( const sc_logic_vector& x, const char* y )
{
    return *(sc_logic_vector::get_concat_array( (sc_logic_vector*) &x, y ));
}
...
const sc_logic_vector&
operator,( const sc_logic_vector& x, const sc_logic_vector& y )
{
    return *(sc_logic_vector::get_concat_array( (sc_logic_vector*) &x, (sc_logic_vector*) &y ));
}

const sc_logic_vector&
operator,( const sc_logic_vector& x, const sc_logic& y )
{
    return *(sc_logic_vector::get_concat_array( (sc_logic_vector*) &x, (sc_logic*) &y ));
}
...
const sc_logic_vector&
operator,( const sc_logic_vector& x, const sc_signal<sc_logic>& y )
{
    return *(sc_logic_vector::get_concat_array( (sc_logic_vector*) &x, (sc_logic*) y.get_data_ptr() ));
}
...
signal_sulv*
signal_sulv::get_concat_array( signal_sulv* x, signal_sulv* y )
{
    return (signal_sulv*) sc_array_base::get_concat_array( x, y, new_array_vv );
}

signal_sulv*
signal_sulv::get_concat_array( signal_sulv* x, sc_signal<sc_logic>* y )
{
    return (signal_sulv*) sc_array_base::get_concat_array( x, (void*) y, new_array_vs );
}

signal_sulv*
signal_sulv::get_concat_array( sc_signal<sc_logic>* x, signal_sulv* y )
{
    return (signal_sulv*) sc_array_base::get_concat_array( (void*) x, y, new_array_sv );
}

signal_sulv*
signal_sulv::get_concat_array( sc_signal<sc_logic>* x, sc_signal<sc_logic>* y )
{
    return (signal_sulv*) sc_array_base::get_concat_array( (void*) x, (void*) y, new_array_ss );
}

sc_array_base*
signal_sulv::new_array_vv( sc_array_base* x, sc_array_base* y )
{
    return new signal_sulv( (sc_signal_logic_vector*) x, (sc_signal_logic_vector*) y );
}

sc_array_base*
signal_sulv::new_array_vs( sc_array_base* x, void* y )
{
    return new signal_sulv( (sc_signal_logic_vector*) x, (sc_signal<sc_logic>*) y );
}

sc_array_base*
signal_sulv::new_array_sv( void* x, sc_array_base* y )
{
    return new signal_sulv( (sc_signal<sc_logic>*) x, (sc_signal_logic_vector*) y );
}

sc_array_base*
signal_sulv::new_array_ss( void* x, void* y )
{
    return new signal_sulv( (sc_signal<sc_logic>*) x, (sc_signal<sc_logic>*) y );
}


void
signal_sulv::print(ostream& os) const
{
    os << read();
}

void
signal_sulv::dump(ostream& os) const
{
    int i;
    int len = length();
...
signal_sulv&
operator,( signal_sulv& x, signal_sulv& y )
{
    return *(signal_sulv::get_concat_array( &x, &y ));
}

signal_sulv&
operator,( signal_sulv& x, sc_signal<sc_logic>& y )
{
    return *(signal_sulv::get_concat_array( &x, &y ));
}

signal_sulv&
operator,( sc_signal<sc_logic>& x, signal_sulv& y )
{
    return *(signal_sulv::get_concat_array( &x, &y ));
}

const signal_sulv&
operator,( const signal_sulv& x, const signal_sulv& y )
{
    return *(signal_sulv::get_concat_array( (signal_sulv*) &x, (signal_sulv*) &y ));
}

const signal_sulv&
operator,( const signal_sulv& x, const sc_signal<sc_logic>& y )
{
    return *(signal_sulv::get_concat_array( (signal_sulv*) &x, (sc_signal<sc_logic>*) &y ));
}

const signal_sulv&
operator,( const sc_signal<sc_logic>& x, const signal_sulv& y )
{
    return *(signal_sulv::get_concat_array( (sc_signal<sc_logic>*) &x, (signal_sulv*) &y ));
}
